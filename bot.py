...
class MonopolyProperty:
    def __init__(self, name, price, rent, color, position_type='property'):
        self.name = name
        self.price = price
        self.rent = rent  # Base rent (list for houses: [0 houses, 1, 2, 3, 4, hotel])
        self.color = color
        self.owner = None
        self.houses = 0
...
def create_monopoly_board():
    """Creates the classic Monopoly board."""
    board = [
        MonopolyProperty("GO", 0, [0], None, 'corner'),
        MonopolyProperty("Mediterranean Avenue", 60, [2, 10, 30, 90, 160, 250], 'brown'),
        MonopolyProperty("Community Chest", 0, [0], None, 'community'),
        MonopolyProperty("Baltic Avenue", 60, [4, 20, 60, 180, 320, 450], 'brown'),
        MonopolyProperty("Income Tax", 200, [0], None, 'tax'),
        MonopolyProperty("Reading Railroad", 200, [25, 50, 100, 200], 'railroad', 'railroad'),
        MonopolyProperty("Oriental Avenue", 100, [6, 30, 90, 270, 400, 550], 'lightblue'),
        MonopolyProperty("Chance", 0, [0], None, 'chance'),
        MonopolyProperty("Vermont Avenue", 100, [6, 30, 90, 270, 400, 550], 'lightblue'),
        MonopolyProperty("Connecticut Avenue", 120, [8, 40, 100, 300, 450, 600], 'lightblue'),
        MonopolyProperty("Just Visiting/Jail", 0, [0], None, 'corner'),
        MonopolyProperty("St. Charles Place", 140, [10, 50, 150, 450, 625, 750], 'pink'),
        MonopolyProperty("Electric Company", 150, [0], 'utility', 'utility'),
        MonopolyProperty("States Avenue", 140, [10, 50, 150, 450, 625, 750], 'pink'),
        MonopolyProperty("Virginia Avenue", 160, [12, 60, 180, 500, 700, 900], 'pink'),
        MonopolyProperty("Pennsylvania Railroad", 200, [25, 50, 100, 200], 'railroad', 'railroad'),
        MonopolyProperty("St. James Place", 180, [14, 70, 200, 550, 750, 950], 'orange'),
        MonopolyProperty("Community Chest", 0, [0], None, 'community'),
        MonopolyProperty("Tennessee Avenue", 180, [14, 70, 200, 550, 750, 950], 'orange'),
        MonopolyProperty("New York Avenue", 200, [16, 80, 220, 600, 800, 1000], 'orange'),
        MonopolyProperty("Free Parking", 0, [0], None, 'corner'),
        MonopolyProperty("Kentucky Avenue", 220, [18, 90, 250, 700, 875, 1050], 'red'),
        MonopolyProperty("Chance", 0, [0], None, 'chance'),
        MonopolyProperty("Indiana Avenue", 220, [18, 90, 250, 700, 875, 1050], 'red'),
        MonopolyProperty("Illinois Avenue", 240, [20, 100, 300, 750, 925, 1100], 'red'),
        MonopolyProperty("B&O Railroad", 200, [25, 50, 100, 200], 'railroad', 'railroad'),
        MonopolyProperty("Atlantic Avenue", 260, [22, 110, 330, 800, 975, 1150], 'yellow'),
        MonopolyProperty("Ventnor Avenue", 260, [22, 110, 330, 800, 975, 1150], 'yellow'),
        MonopolyProperty("Water Works", 150, [0], 'utility', 'utility'),
        MonopolyProperty("Marvin Gardens", 280, [24, 120, 360, 850, 1025, 1200], 'yellow'),
        MonopolyProperty("Go To Jail", 0, [0], None, 'gotojail'),
        MonopolyProperty("Pacific Avenue", 300, [26, 130, 390, 900, 1100, 1275], 'green'),
        MonopolyProperty("North Carolina Avenue", 300, [26, 130, 390, 900, 1100, 1275], 'green'),
        MonopolyProperty("Community Chest", 0, [0], None, 'community'),
        MonopolyProperty("Pennsylvania Avenue", 320, [28, 150, 450, 1000, 1200, 1400], 'green'),
        MonopolyProperty("Short Line Railroad", 200, [25, 50, 100, 200], 'railroad', 'railroad'),
        MonopolyProperty("Chance", 0, [0], None, 'chance'),
        MonopolyProperty("Park Place", 350, [35, 175, 500, 1100, 1300, 1500], 'darkblue'),
        MonopolyProperty("Luxury Tax", 100, [0], None, 'tax'),
        MonopolyProperty("Boardwalk", 400, [50, 200, 600, 1400, 1700, 2000], 'darkblue'),
    ]
    return board

def get_chance_card():
    """Returns a random Chance card effect."""
    cards = [
        ...
        ("Advance to Boardwalk", 'move', 39),
        ("Advance to Illinois Avenue", 'move', 24),
        ("Get Out of Jail Free", 'jailcard'),
    ]
    return random.choice(cards)

def get_community_chest_card():
    """Returns a random Community Chest card effect."""
    cards = [
        ("Advance to Go (Collect $200)", 'go'),
        ("Bank error in your favor – Collect $200", 'money', 200),
        ("Doctor's fees – Pay $50", 'pay', 50),
        ("From sale of stock you get $50", 'money', 50),
        ("Get Out of Jail Free", 'jailcard'),
        ("Go to Jail", 'jail'),
        ("Grand Opera Night – Collect $50 from every player", 'collect', 50),
...
def get_color_emoji(color):
    """Returns emoji for property color."""
    color_map = {
        'brown': '🟤',
        'lightblue': '🔵',
        'pink': '🩷',
...
def draw_monopoly_board_text(session):
    """Creates a text representation of the Monopoly board."""
    board = session['board']
    players = session['players']
    
    # Create player position markers
    position_markers = {}
    for uid, player in players.items():
        pos = player.position
        if pos not in position_markers:
            position_markers[pos] = []
        try:
            position_markers[pos].append(uid[:2])  # First 2 chars of user ID
        except:
            position_markers[pos].append('P')
    
    # Top row (positions 20-30)
    top_row = "```\n"
    for i in range(20, 31):
        marker = ','.join(position_markers.get(i, [''])[:2]) if i in position_markers else '  '
        top_row += f"[{i:2d}:{marker:4s}]"
    top_row += "\n"
    
    # Middle section (left and right sides)
    for row in range(9):
        left_i = 19 - row
        right_i = 31 + row
        
        left_marker = ','.join(position_markers.get(left_i, [''])[:2]) if left_i in position_markers else '  '
        right_marker = ','.join(position_markers.get(right_i, [''])[:2]) if right_i in position_markers else '  '
        
        top_row += f"[{left_i:2d}:{left_marker:4s}]"
        top_row += " " * 77
        top_row += f"[{right_i:2d}:{right_marker:4s}]\n"
...
async def monopoly(ctx, action: str = None):
    ...
    Usage: !monopoly start - Host starts the game
           !monopoly join - Players join
           !monopoly begin - Start the game
           !monopoly roll - Roll dice
           !monopoly buy - Buy property
           !monopoly board - View board
           !monopoly status - Game status
           !monopoly properties - Your properties
    ...
    if action is None:
        await ctx.send(
            "**🎲 Classic Monopoly**\n"
            "Commands:\n"
            "`!monopoly start` - Start a new game\n"
            "`!monopoly join` - Join the lobby\n"
            "`!monopoly begin` - Begin (host only)\n"
            "`!monopoly roll` - Roll dice\n"
            "`!monopoly buy` - Buy property\n"
            "`!monopoly board` - View board\n"
            "`!monopoly status` - Game status\n"
    ...
    if action == "start":
        ...
        embed = discord.Embed(
            ...
            description=f"**Players:** {len(session['players'])}\n"
                       f"Starting balance: $1500 each\n\n"
                       f"**Turn Order:**\n" + 
        ...
        if player.in_jail:
            ...
            if dice1 == dice2:
                ...
                player.jail_turns = 0
                await ctx.send(f"🎲 {ctx.author.mention} rolled doubles ({dice1}, {dice2}) and got out of jail!")
                ...
                if player.jail_turns >= 3:
                    ...
                    player.in_jail = False
                    player.jail_turns = 0
                    await ctx.send(f"🎲 {ctx.author.mention} paid $50 to get out of jail after 3 turns!")
                else:
                    await ctx.send(f"🎲 {ctx.author.mention} rolled ({dice1}, {dice2}) - not doubles. Still in jail ({3 - player.jail_turns} turns left)")
        ...
        if is_doubles:
            ...
            if session['doubles_count'] >= 3:
                ...
                await ctx.send(f"🎲 {ctx.author.mention} rolled 3 doubles in a row! Go to Jail!")
                session['current_turn'] = (session['current_turn'] + 1) % len(session['turn_order'])
                return
        else:
            session['doubles_count'] = 0
        ...
        if player.position < old_pos:
            player.balance += 200
            await ctx.send(f"💰 {ctx.author.mention} passed GO! Collected $200")
        
        space = session['board'][player.position]
        
        embed = discord.Embed(
            title=f"🎲 {ctx.author.display_name} rolled {dice1} + {dice2} = {total}{'🎲 DOUBLES!' if is_doubles else ''}",
            color=discord.Color.blue()
        )
        
        # Add color emoji if property
        space_display = space.name
        if space.position_type in ['property', 'railroad', 'utility']:
            space_display = f"{get_color_emoji(space.color)} {space.name}"
        
        embed.add_field(name="Landed on", value=f"**{space_display}**", inline=False)
        
        # Handle different space types
        if space.position_type == 'corner':
            if 'GO' in space.name and player.position == 0:
                embed.add_field(name="💰", value="Landed on GO!", inline=False)
        ...
        elif space.position_type == 'gotojail':
            player.in_jail = True
            player.jail_turns = 0
            player.position = 10
            embed.add_field(name="🚔 Go to Jail!", value="Sent to jail!", inline=False)
        
        elif space.position_type == 'tax':
            tax_amount = 200 if 'Income' in space.name else 100
            player.balance -= tax_amount
            session['pot'] += tax_amount
            embed.add_field(name="💸 Tax", value=f"Paid ${tax_amount}", inline=False)
        
        elif space.position_type == 'chance':
            card = get_chance_card()
            embed.add_field(name="🎴 Chance", value=card[0], inline=False)
            await handle_card(ctx, session, player, card, uid)
        
        elif space.position_type == 'community':
            card = get_community_chest_card()
            embed.add_field(name="📦 Community Chest", value=card[0], inline=False)
            await handle_card(ctx, session, player, card, uid)
        
        elif space.position_type in ['property', 'railroad', 'utility']:
            if space.owner is None:
                embed.add_field(
                    name="🏷️ For Sale",
                    value=f"**Price:** ${space.price}\n**Rent:** ${space.rent[0] if space.position_type == 'property' else space.rent[0]}\n\n",
                    inline=False
                )
                embed.set_footer(text="React with 💰 to buy or ❌ to pass (60s)")
                ...
                session['pending_purchase'] = {
                    'player_id': uid,
                    'property_pos': player.position,
                    'message_id': msg.id
                }
                
                def check(reaction, user):
                    return (str(user.id) == uid and 
                ...
                try:
                    ...
                    if str(reaction.emoji) == '💰':
                        if player.balance >= space.price:
                            ...
                            space.owner = uid
                            player.owned_properties.append(player.position)
                            await ctx.send(f"✅ {ctx.author.mention} bought **{space.name}** for ${space.price}!")
                        else:
                            await ctx.send(f"❌ Not enough money! Need ${space.price}, you have ${player.balance}")
                    else:
                        await ctx.send(f"{ctx.author.mention} passed on buying {space.name}")
                except asyncio.TimeoutError:
                    await ctx.send(f"⏰ {ctx.author.mention} didn't respond in time. Passed on {space.name}")
                
                session['pending_purchase'] = None
                
            elif space.owner == uid:
                embed.add_field(name="✅ Your Property", value="You own this property!", inline=False)
                await ctx.send(embed=embed)
            else:
                # Calculate rent
                rent = space.rent[space.houses] if space.position_type == 'property' else space.rent[0]
                
                if space.position_type == 'railroad':
                    railroads_owned = sum(1 for p in session['board'] if p.position_type == 'railroad' and p.owner == space.owner)
                    rent = space.rent[railroads_owned - 1]
                elif space.position_type == 'utility':
                    utilities_owned = sum(1 for p in session['board'] if p.position_type == 'utility' and p.owner == space.owner)
                    rent = total * (4 if utilities_owned == 1 else 10)
                ...
                try:
                    ...
                    embed.add_field(name="💸 Rent Paid", value=f"Paid ${rent} to {owner_user.mention}", inline=False)
                except:
        ...
        embed2.add_field(name="💰 Balance", value=f"${player.balance}", inline=True)
        embed2.add_field(name="📍 Position", value=f"{player.position}", inline=True)
        ...
        if player.balance < 0:
            ...
            if len(active_players) == 1:
                ...
                try:
                    winner = await bot.fetch_user(int(winner_id))
                    await ctx.send(f"🏆 **GAME OVER!** {winner.mention} wins Monopoly!")
                except:
        ...
        if not is_doubles or player.in_jail:
            ...
            await ctx.send(f"🎲 {ctx.author.mention} rolled doubles! Roll again!")
        ...
        embed = discord.Embed(title=f"🏠 {ctx.author.display_name}'s Properties", color=discord.Color.blue())
        
        if not player.owned_properties:
            embed.description = "You don't own any properties yet!"
        else:
            props_by_color = {}
            for pos in player.owned_properties:
                prop = session['board'][pos]
                color = prop.color or prop.position_type
                if color not in props_by_color:
                    props_by_color[color] = []
                house_info = f" [{prop.houses} 🏠]" if prop.position_type == 'property' and prop.houses > 0 else ""
                props_by_color[color].append(f"{get_color_emoji(color)} {prop.name}{house_info}")
            
            for color, props in props_by_color.items():
                embed.add_field(name=color.title(), value="\n".join(props), inline=False)
        
        embed.add_field(name="💰 Balance", value=f"${player.balance}", inline=True)
        embed.add_field(name="📊 Total Properties", value=str(len(player.owned_properties)), inline=True)
        ...
        session = monopoly_sessions[ch]
        board_text = draw_monopoly_board_text(session)
        
        await ctx.send("**🎲 Monopoly Board**\n" + board_text)
        await ctx.send("Use `!monopoly status` to see property details")
        return
        ...
        embed = discord.Embed(title="🎲 Monopoly - Status", color=discord.Color.blue())
        ...
        for uid in session['turn_order']:
            ...
            try:
                ...
                embed.add_field(
                    ...
                    value=f"Balance: {status}\nPosition: {player.position}\nProperties: {len(player.owned_properties)}",
                    inline=False
                )
            except:
                pass
        ...
        await ctx.send("🎲 Monopoly game ended.")
        return
    
    else:
        await ctx.send("Unknown action. Use: start, join, begin, roll, properties, board, status, or end")


async def handle_card(ctx, session, player, card, uid):
    """Handles Chance and Community Chest card effects."""
    card_type = card[1]
    
    if card_type == 'go':
        player.position = 0
        player.balance += 200
        await ctx.send(f"💰 {ctx.author.mention} advanced to GO and collected $200!")
    
    elif card_type == 'money':
        amount = card[2]
        player.balance += amount
        await ctx.send(f"💰 {ctx.author.mention} received ${amount}!")
    
    elif card_type == 'pay':
        amount = card[2]
        player.balance -= amount
        session['pot'] += amount
        await ctx.send(f"💸 {ctx.author.mention} paid ${amount}!")
    
    elif card_type == 'jail':
        player.in_jail = True
        player.jail_turns = 0
        player.position = 10
        await ctx.send(f"🚔 {ctx.author.mention} goes to Jail!")
    
    elif card_type == 'jailcard':
        player.get_out_of_jail_cards += 1
        await ctx.send(f"🎫 {ctx.author.mention} got a Get Out of Jail Free card!")
    
    elif card_type == 'back':
        spaces = card[2]
        player.position = (player.position - spaces) % 40
        await ctx.send(f"⏪ {ctx.author.mention} moved back {spaces} spaces to position {player.position}!")
    
    elif card_type == 'move':
        new_pos = card[2]
        if new_pos < player.position:  # Passed GO
            player.balance += 200
            await ctx.send(f"💰 Passed GO! Collected $200")
        player.position = new_pos
        await ctx.send(f"➡️ {ctx.author.mention} moved to position {player.position}!")
    
    elif card_type == 'collect':
        amount = card[2]
        for other_uid, other_player in session['players'].items():
            if other_uid != uid and not other_player.bankrupt:
                other_player.balance -= amount
                player.balance += amount
        await ctx.send(f"💰 {ctx.author.mention} collected ${amount} from each player!")
...
if __name__ == "__main__":
    bot.run(os.getenv("DISCORD_BOT_TOKEN"))import discord
