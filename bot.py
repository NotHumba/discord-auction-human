    ...
    if (auction_state_for_channel
        ...
        if set_key in available_sets:
            ...
            if all_positions_loaded_successfully:
                ...
                embed = discord.Embed(
                    title="Ã¢Å¡ Ã¯Â¸Â Set Loaded with Warnings",
                    description=
    ...
    if (lineup_setup_state['user_id'] == str(message.author.id)
        ...
        if lineup_setup_state['stage'] == 'formation':
            ...
            if formation in available_formations:
                ...
                embed = discord.Embed(title="Ã°Å¸Å½Â¯ Select Tactic",
            ...
            if matched_player in lineup_setup_state['selected_players']:
                embed = discord.Embed(
                    title="Ã¢ÂÅ’ Player Already Selected",
                    description=
            ...
            if next_pos:
                ...
                if not save_data():
                    await message.channel.send(
                        "Ã¢Å¡ Ã¯Â¸Â Error saving lineup data. Please try again.")
                embed = discord.Embed(title="Ã¢Å“â€¦ Lineup Set",
                                      color=discord.Color.green())
...
async def prompt_for_player(channel, user, position):
    ...
    if not available_players:
        embed = discord.Embed(
            title="Ã¢ÂÅ’ No Players Available",
            description=
    ...
    embed = discord.Embed(
        title=f"Ã°Å¸â€œâ€¹ Select {position.upper()} ({count_needed} needed)",
        description=
...
async def check_host_activity(channel_id):
    ...
    while channel_id in active_auctions:
        ...
        if current_time - last_activity > HOST_TIMEOUT:
            ...
            if channel:
                await channel.send(
                    "Ã°Å¸â€Å¡ Auction ended due to host inactivity for 5 minutes. Participant budgets, teams, and lineups have been reset."
                )
...
async def startauction(ctx, *members: discord.Member, timer: int = 30):
    ...
    if ctx.channel.id in active_auctions:
        await ctx.send(
            "Ã¢ÂÅ’ An auction is already active in this channel. Please use a different channel or end the current auction first."
        )
        return
    ...
    if is_user_in_any_auction(
        ...
        await ctx.send(
            f"Ã¢ÂÅ’ {ctx.author.display_name}, you are already participating in another auction."
        )
        return
    ...
    for member in members:
        if is_user_in_any_auction(
            ...
            await ctx.send(
                f"Ã¢ÂÅ’ {member.display_name} is already participating in another auction."
            )
            return
    ...
    embed = discord.Embed(
        title="Ã°Å¸Å½Â¯ Select Auction Set",
...
async def sets(ctx):
    ...
    embed = discord.Embed(title="Ã°Å¸Å½Â¯ Available Auction Sets",
...
async def participants(ctx):
    ...
    embed = discord.Embed(title="Ã°Å¸â€˜Â¥ Registered Participants",
                          description="\n".join(users),
                          color=discord.Color.green())
...
async def add(ctx, member: discord.Member):
    ...
    await ctx.send(f"Ã¢Å“â€¦ {member.mention} has been added to this auction.")
...
async def remove(ctx, member: discord.Member):
    ...
    if str(member.id) not in auction_state['participants']:
        await ctx.send(
            f"Ã¢ÂÅ’ {member.mention} is not a participant in this auction.")
        return

    confirm_msg = await ctx.send(
        f"Ã¢Å¡ Ã¯Â¸Â Are you sure you want to remove {member.mention} from this auction? React with Ã¢Å“â€¦ to confirm."
    )
    await confirm_msg.add_reaction("Ã¢Å“â€¦")

    def check(reaction, user):
        return user == ctx.author and str(
            reaction.emoji) == "Ã¢Å“â€¦" and reaction.message.id == confirm_msg.id

    try:
        await bot.wait_for('reaction_add', timeout=15.0, check=check)
        auction_state['participants'].remove(str(member.id))
        await ctx.send(
            f"Ã¢ÂÅ’ {member.mention} has been removed from this auction.")
    except asyncio.TimeoutError:
        await ctx.send("Ã¢ÂÂ° Removal cancelled. No confirmation received in time."
                       )
...
async def setlineup(ctx, lineup_name: str = 'main'):
    ...
    embed = discord.Embed(
        title="Ã°Å¸Å½Â¯ Select Formation",
    ...
    try:
        ...
        if lineup_setup_state['user_id'] == user_id and lineup_setup_state[
                'stage'] is not None:
            await ctx.send(
                "Ã¢ÂÂ° Lineup setup timed out. Please run !setlineup again.")
            reset_lineup_setup_state()
...
async def viewlineup(ctx):
    ...
    embed = discord.Embed(title=f"Ã°Å¸â€œâ€¹ {ctx.author.display_name}'s Lineup",
...
def create_position_command(position):
    ...
    async def _position(ctx):
        ...
        if auction_state['current_set'] is None:
            await ctx.send(
                "Ã¢ÂÅ’ No set has been selected for this auction. The host needs to select a set first."
            )
            return
        ...
        embed = discord.Embed(title="Ã°Å¸â€Â¨ Player Up for Auction",
        ...
        embed.set_footer(
            text=
            "Use !bid or !bid [amount] to place a bid. React with Ã°Å¸â€™Â° to bid, Ã¢ÂÅ’ to pass."
        )

        message = await ctx.send(embed=embed)
        await message.add_reaction("Ã°Å¸â€™Â°")
        await message.add_reaction("Ã¢ÂÅ’")
            ...
            try:
                ...
                if not auction_state.get('bidding',
                                         False) or auction_state.get(
                                             'current_player') != player:
                    return
                await ctx.send("Ã¢Å’â€º Going once...")
                await asyncio.sleep(1)
                if not auction_state.get('bidding',
                                         False) or auction_state.get(
                                             'current_player') != player:
                    return
                await ctx.send("Ã¢Å’â€º Going twice...")
                await asyncio.sleep(1)
                if not auction_state.get('bidding',
                                         False) or auction_state.get(
                                             'current_player') != player:
                    return
                await ctx.send("Ã¢Å’â€º Final call...")
                await asyncio.sleep(1)
                if not auction_state.get('bidding',
                                         False) or auction_state.get(
                                             'current_player') != player:
...
async def bid(ctx, *args):
    ...
    if len(args) >= 2:
        ...
        try:
            ...
            await ctx.send(
                "Ã¢ÂÅ’ Invalid price format. Use numbers like 10m or 1000000.")
            return
        ...
        for user_id, team in user_teams.items():
            for player in team:
                if player['name'].lower() == player_name.lower():
                    await ctx.send(
                        f"Ã¢ÂÅ’ Player **{player_name}** has already been sold.")
                    return

        if player_name.lower() in auction_state['unsold_players']:
            await ctx.send(
                f"Ã¢ÂÅ’ Player **{player_name}** was previously marked as unsold in this auction."
            )
            return
        ...
        embed = discord.Embed(title="Ã°Å¸â€Â¨ Custom Player Up for Auction",
                              color=discord.Color.gold())
    ...
    if len(args) == 1:
        ...
        try:
            ...
            await ctx.send(
                "Ã¢ÂÅ’ Invalid bid amount format. Use numbers like 50m or 1000000."
            )
            return
        ...
        if new_price < auction_state[
                'current_price'] + MIN_BID_INCREMENT and new_price != auction_state[
                    'current_price']:
            await ctx.send(
                f"Ã¢ÂÅ’ Minimum bid increment is {format_currency(MIN_BID_INCREMENT)}."
            )
            return
    else:
    ...
    await ctx.send(
        f"Ã°Å¸Å¸Â¡ {ctx.author.display_name} bids {format_currency(new_price)}!")
...
async def _finalize_sold(ctx):
    ...
    if auction_state['highest_bidder'] is None:
        ...
        auction_state['pass_votes'].clear()
        await ctx.send(
            f"Ã¢ÂÅ’ No one bid for **{player['name']}**. They go unsold.")
        return
    ...
    if not save_data():
        await ctx.send(
            "Ã¢Å¡ Ã¯Â¸Â Error saving data. Sale recorded but data may not persist.")
        return

    embed = discord.Embed(title="Ã¢Å“â€¦ Player Sold!", color=discord.Color.green())
...
async def rebid(ctx):
    ...
    if not save_data():
        await ctx.send(
            "Ã¢Å¡ Ã¯Â¸Â Error saving data. Rebid proceeding, but data may not persist."
        )
    ...
    embed = discord.Embed(title="Ã°Å¸â€Â¨ Player Re-Auction",
                          color=discord.Color.gold())
    embed.add_field(name="Name", value=player['name'], inline=True)
    embed.add_field(name="Position",
                    value=player.get('position', 'Unknown').upper(),
    ...
    auction_state['timeout_task'] = bot.loop.create_task(auto_sold())
    await ctx.send(
        f"Ã¢Å“â€¦ **{player['name']}** is being re-auctioned. Previous buyer <@{buyer_id}> has been refunded {format_currency(price)}."
    )
...
async def status(ctx):
    ...
    if not auction_state['bidding'] or not auction_state['current_player']:
        await ctx.send(
            "Ã¢Å¡ Ã¯Â¸Â No player is currently being auctioned in this channel.")
        return
    ...
    embed = discord.Embed(title="Ã°Å¸â€œÂ¢ Current Auction Status",
                          color=discord.Color.blue())
    embed.add_field(name="Player", value=player['name'], inline=True)
    embed.add_field(name="Position",
                    value=player.get('position', 'Unknown').upper(),
...
async def unsold(ctx):
    ...
    await ctx.send(
        f"Ã¢ÂÅ’ Player **{player['name']}** goes unsold in this auction.")
...
async def myplayers(ctx):
    ...
    embed = discord.Embed(title=f"Ã°Å¸â€œâ€¹ {ctx.author.display_name}'s Players",
                          color=discord.Color.teal())
...
async def budget(ctx):
    ...
    await ctx.send(f"Ã°Å¸â€™Â° Your remaining budget: {format_currency(budget)}")
    ...
    for _ in range(events):
        ...
        if event == 'goal':
            if pos in ['ST', 'LW', 'RW', 'CAM']:
                narrative.append(
                    f"Ã¢Å¡Â½ {player_name} ({pos}) scores a {random.choice(['stunning', 'clinical', 'brilliant'])} goal for {team_name_display}!"
                )
            else:
                narrative.append(
                    f"Ã¢Å¡Â½ {player_name} ({pos}) scores a rare goal for {team_name_display}!"
                )
        elif event == 'save':
            if pos == 'GK':
                narrative.append(
                    f"Ã°Å¸Â§Â¤ {player_name} ({pos}) makes a fantastic save to deny {team_name_display}'s opponent!"
                )
            else:
                narrative.append(
                    f"Ã°Å¸Â§Â¤ {team_name_display}'s goalkeeper makes a crucial save!"
                )
        elif event == 'chance':
            if pos in ['ST', 'LW', 'RW', 'CAM']:
                narrative.append(
                    f"Ã°Å¸Å½Â¯ {player_name} ({pos}) misses a golden opportunity for {team_name_display}!"
                )
            else:
                narrative.append(
                    f"Ã°Å¸Å½Â¯ {player_name} ({pos}) creates a chance for {team_name_display}!"
                )
        elif event == 'tackle':
            if pos in ['CB', 'LB', 'RB', 'CM']:
                narrative.append(
                    f"Ã°Å¸â€™Âª {player_name} ({pos}) makes a crunching tackle to stop {team_name_display}'s opponent!"
                )
            else:
                narrative.append(
                    f"Ã°Å¸â€™Âª {player_name} ({pos}) makes a key defensive play for {team_name_display}!"
                )
        elif event == 'assist':
            if pos in ['CAM', 'LW', 'RW', 'CM']:
                narrative.append(
                    f"Ã°Å¸Å½Â {player_name} ({pos}) delivers a perfect assist for {team_name_display}!"
                )
            else:
                narrative.append(
                    f"Ã°Å¸Å½Â {player_name} ({pos}) sets up a goal for {team_name_display}!"
                )
...
async def battle(ctx, team1: discord.Member, team2: discord.Member):
    ...
    embed = discord.Embed(title="Ã¢Å¡Â½ Match Result", color=discord.Color.purple())
    ...
    if team1_goals > team2_goals:
        embed.add_field(name="Winner",
                        value=f"{team1.display_name} Ã°Å¸Ââ€ ",
                        inline=False)
    elif team2_goals > team1_goals:
        embed.add_field(name="Winner",
                        value=f"{team2.display_name} Ã°Å¸Ââ€ ",
                        inline=False)
    else:
        embed.add_field(name="Result", value="Draw Ã°Å¸Â¤Â", inline=False)
...
async def rankteams(ctx):
    ...
    embed = discord.Embed(title="Ã°Å¸Ââ€  Team Rankings (Based on Lineup)",
                          color=discord.Color.gold())
    description_list = []
...
async def endauction(ctx):
    ...
    if not save_data():
        await ctx.send(
            "Ã¢Å¡ Ã¯Â¸Â Error saving data. Auction ended, but data may not persist.")
        return

    await ctx.send(
        "Ã°Å¸â€Å¡ Auction in this channel has been ended. Participant budgets, teams, and lineups have been reset."
    )

@bot.command()
async def footy(ctx, category: str = None):
    """Shows categorized help for the bot."""
    embed = discord.Embed(title="Ã°Å¸â€œËœ Football Auction Bot Commands",
                          color=discord.Color.blue())
    ...
    if category is None:
        embed.description = (
            ...
            "Ã¢Å¡Â½ auction, Ã°Å¸â€˜Â¥ team, Ã°Å¸Å½Â® gamemodes, Ã°Å¸â€œÅ  leaderboards")
    elif category.lower() == "auction":
        embed.add_field(
            name="Auction Commands",
            value="\n".join([
                "`!startauction @users...` Ã¢â‚¬â€œ Start a new auction",
                "`!sets` Ã¢â‚¬â€œ Show all available sets",
                "`!participants` Ã¢â‚¬â€œ List participants",
                "`!bid [amount]` Ã¢â‚¬â€œ Place a bid",
                "`!sold / !unsold` Ã¢â‚¬â€œ Resolve auction",
                "`!status` Ã¢â‚¬â€œ Current auction status",
                "`!endauction` Ã¢â‚¬â€œ End current auction"
        ]),
        inline=False)
    elif category.lower() == "team":
        embed.add_field(name="Team Commands",
                        value="\n".join([
                            "`!myplayers` Ã¢â‚¬â€œ View your bought players",
                            "`!budget` Ã¢â‚¬â€œ Show your budget",
                            "`!setlineup [name]` Ã¢â‚¬â€œ Setup/edit a lineup",
                            "`!lineups` Ã¢â‚¬â€œ View all your lineups",
                            "`!switchlineup <name>` Ã¢â‚¬â€œ Switch active lineup",
                            "`!deletelineup <name>` Ã¢â‚¬â€œ Delete a lineup",
                            "`!viewlineup` Ã¢â‚¬â€œ View your active lineup",
                            "`!battle @user1 @user2` Ã¢â‚¬â€œ Simulate a match"
                        ]),
                        inline=False)
        ...
        embed.add_field(
            name="Gamemodes",
            value="\n".join([
                "`!koth start` Ã¢â‚¬â€œ Start King of the Hill (max 6 players)",
                "`!draftclash start` Ã¢â‚¬â€œ Start Draft Clash (max 8 players)",
                "`!draftclash koth` Ã¢â‚¬â€œ Enter KoTH with drafted team",
                "`!challenge @user` Ã¢â‚¬â€œ Challenge for the throne",
                "`!end <gamemode>` Ã¢â‚¬â€œ End active game sessions"
            ]),
            inline=False)
        ...
        embed.add_field(
            name="Leaderboards",
            value="\n".join([
                "`!leaderboard auction` Ã¢â‚¬â€œ Auction stats",
                "`!leaderboard gamemodes` Ã¢â‚¬â€œ KoTH & Draft Clash stats",
                "`!draftclashleaderboard` Ã¢â‚¬â€œ Draft Clash wins"
            ]),
            inline=False)
    else:
        embed.description = "Ã¢ÂÅ’ Unknown category. Try: auction, team, gamemodes, leaderboards."
...
async def market(ctx):
    ...
    embed = discord.Embed(title="Ã°Å¸â€ºâ€™ Free Agent Market",
                          color=discord.Color.gold())
...
async def events(ctx):
    ...
    events_list = [
        "Ã°Å¸Å¡â€˜ Injury Ã¢â‚¬â€œ One of your players is out for 2 matches!",
        "Ã¢Å¡Â¡ Form Boost Ã¢â‚¬â€œ Random player gains +2 form for 3 matches!",
        "Ã°Å¸â€â€ž Transfer Rumor Ã¢â‚¬â€œ Random player may be swapped with the market!",
    ]
    event = random.choice(events_list)
    await ctx.send(f"Ã°Å¸Å½Â² Random Event: {event}")
...
async def draft(ctx):
    ...
    await ctx.send(
        "Ã°Å¸â€œâ€¹ Use `!draft start` to begin the draft or `!draft pick <player>` to pick a player."
    )


@draft.command()
async def start(ctx):
    await ctx.send("Ã°Å¸â€œâ€¹ Draft mode started! Turn order will be assigned.")


@draft.command()
async def pick(ctx, *, player_name):
    await ctx.send(f"Ã¢Å“â€¦ {ctx.author.mention} picked **{player_name}**.")
...
EMOJI_NUMBERS = ['1Ã¯Â¸ÂÃ¢Æ’Â£', '2Ã¯Â¸ÂÃ¢Æ’Â£', '3Ã¯Â¸ÂÃ¢Æ’Â£', '4Ã¯Â¸ÂÃ¢Æ’Â£']
...
async def draftclash(ctx, action: str = None, set_key: str = None):
    ...
    join_msg = await ctx.send(
        f"Draft Clash lobby created by {ctx.author.mention}. React with Ã¢Å“â€¦ to join! Host uses `!draftclash begin <set_key>` to start."
    )
    await join_msg.add_reaction("Ã¢Å“â€¦")

    def check(reaction, user):
        return str(reaction.emoji) == "Ã¢Å“â€¦" and reaction.message.id == join_msg.id and not user.bot
...
async def _draft_run_knockout(ctx, session):
    ...
    while len(bracket) > 1:
        ...
        for i in range(0, len(bracket), 2):
            ...
            nxt.append(winner)
            await ctx.send(
                f"Round {round_no}: <@{a}> vs <@{b}> Ã¢â‚¬â€ Winner: <@{winner}>")
        bracket = nxt
        round_no += 1
    champ = bracket[0] if bracket else None
    if champ: 
        await ctx.send(f"Ã°Å¸ÂÂ Draft Clash Champion: <@{champ}>")
        # Announce KoTH integration
        await ctx.send("Ã°Å¸Å½Â¯ **Draft lineups are now ready for King of the Hill!** All participants can use `!draftclash koth` to enter KoTH battles with their drafted teams.")
...
async def draftclashleaderboard(ctx):
    ...
    items = sorted(draft_clash_wins.items(), key=lambda x: x[1],
                   reverse=True)[:10]
    desc = "\n".join([f"<@{uid}> Ã¢â‚¬â€ {wins} wins" for uid, wins in items])
    embed = discord.Embed(title="Ã¢Å¡Â¡ Draft Clash Leaderboard",
                          description=desc,
                          color=discord.Color.blue())
    await ctx.send(embed=embed)
...
async def lineups(ctx):
    ...
    if user_id not in user_lineups or not user_lineups[user_id]:
        await ctx.send("Ã¢ÂÅ’ You don't have any lineups yet. Use `!setlineup` to create one!")
        return
    
    active_name = active_lineups.get(user_id, 'main')
    embed = discord.Embed(title="Ã°Å¸Å½Â¯ Your Lineups", color=discord.Color.blue())
    ...
    for lineup_name, lineup_data in user_lineups[user_id].items():
        ...
        status = "Ã°Å¸Å¸Â¢ ACTIVE" if lineup_name == active_name else "Ã¢Å¡Âª"
        value = f"{status}\n{player_count}/11 players\n{formation} ({tactic})"
        
        embed.add_field(name=f"Ã°Å¸â€œâ€¹ {lineup_name.title()}", value=value, inline=True)
...
async def switchlineup(ctx, lineup_name: str = None):
    ...
    if lineup_name is None:
        await ctx.send("Ã¢ÂÅ’ Please specify a lineup name: `!switchlineup <name>`")
        return
    ...
    if user_id not in user_lineups or lineup_name not in user_lineups[user_id]:
        await ctx.send(f"Ã¢ÂÅ’ Lineup '{lineup_name}' doesn't exist. Use `!lineups` to see available lineups.")
        return
    ...
    await ctx.send(f"Ã¢Å“â€¦ Switched to lineup: **{lineup_name.title()}**\n"
                  f"Ã°Å¸â€œâ€¹ {player_count}/11 players | {formation} ({tactic})")

@bot.command()
async def deletelineup(ctx, lineup_name: str = None):
    """Delete a saved lineup."""
    if lineup_name is None:
        await ctx.send("Ã¢ÂÅ’ Please specify a lineup name: `!deletelineup <name>`")
        return
    ...
    if lineup_name == 'main':
        await ctx.send("Ã¢ÂÅ’ Cannot delete the main lineup!")
        return
    
    if user_id not in user_lineups or lineup_name not in user_lineups[user_id]:
        await ctx.send(f"Ã¢ÂÅ’ Lineup '{lineup_name}' doesn't exist.")
        return
    ...
    save_data()
    await ctx.send(f"Ã°Å¸â€”â€˜Ã¯Â¸Â Deleted lineup: **{lineup_name.title()}**")
...
async def end(ctx, gamemode: str = None):
    ...
    if gamemode == "koth":
        ...
        save_data()
        await ctx.send("Ã°Å¸â€˜â€˜ King of the Hill session has been ended.")
        ...
        save_data()
        await ctx.send("Ã¢Å¡Â¡ Draft Clash session has been ended.")
        
    else:
        await ctx.send("Ã¢ÂÅ’ Unknown game mode. Available: koth, draftclash")
...
async def koth_add(ctx, *members: discord.Member):
    ...
    if ctx.channel.id in koth_draft and koth_draft[ctx.channel.id]["active"]:
        ...
        await ctx.send("Ã¢Å¡ Ã¯Â¸Â No active KoTH session in this channel!"); return
...
async def koth(ctx, action: str = None, mode: str = None):
    ...
    if action == "start":
        if mode == "auction":
            ...
            await ctx.send(f"Ã°Å¸ÂÅ¸Ã¯Â¸Â Auction KoTH started (ID `{session_id}`). Use `!koth add {session_id} @user1 @user2` to add players. Multiple Auction KoTH allowed in this channel.")
            return
        elif mode == "draftclash":
            if chan in koth_draft and koth_draft[chan].get("active"):
                await ctx.send("Ã¢Å¡ Ã¯Â¸Â A Draft Clash KoTH is already running in this channel!")
                return
            koth_draft[chan] = {
                "id": str(uuid.uuid4())[:8],
                "king": None,
            ...
            await ctx.send("Ã°Å¸Ââ€  Draft Clash KoTH started! Use `!koth add @user1 @user2` to add players. (Draft Clash KoTH uses Draft Clash squads only.)")
            return
        else:
        ...
        for m in ctx.message.mentions:
            ...
            if uid not in user_lineups:
                await ctx.send(f"Ã¢Å¡ Ã¯Â¸Â {m.display_name} has no lineup set. They must set a lineup first.")
                continue
...
async def challenge(ctx, opponent: discord.Member = None):
    ...
    if opponent is None:
        await ctx.send("Ã¢Å¡ Ã¯Â¸Â You must mention someone to challenge. Usage: `!challenge @user`")
        return
    ...
    if not target:
        await ctx.send("Ã¢Å¡ Ã¯Â¸Â No active KoTH session in this channel. Start one with `!koth start auction` or `!koth start draftclash`.")
        return
    ...
    if challenger_id not in players or opponent_id not in players:
        await ctx.send("Ã¢Å¡ Ã¯Â¸Â Both challenger and opponent must be added to the KoTH session via `!koth add`.")
        return
    ...
    if not has_valid_lineup(challenger_id):
        await ctx.send(f"Ã¢Å¡ Ã¯Â¸Â {ctx.author.display_name}, you don't have a valid lineup set. Use `!setlineup` first.")
        return
    if not has_valid_lineup(opponent_id):
        await ctx.send(f"Ã¢Å¡ Ã¯Â¸Â {opponent.display_name} doesn't have a valid lineup set. They must set a lineup with `!setlineup`.")
        return
    ...
    try:
        ...
        await ctx.send(f"Ã¢Å¡ Ã¯Â¸Â Error running match simulation: {e}")
        return

    if not sim:
        await ctx.send("Ã¢Å¡ Ã¯Â¸Â Match could not be simulated.")
        return
    ...
    try:
        ...
        try:
            ...
            await ctx.send("Ã¢Å¡ Ã¯Â¸Â Simulation returned unexpected format; cannot parse result.")
            return
    ...
    title = f"Ã¢Å¡â€Ã¯Â¸Â KoTH{session_label} Ã¢â‚¬â€ {ctx.author.display_name} vs {opponent.display_name}"
    embed = discord.Embed(title=title, color=discord.Color.blurple())
    embed.add_field(name="Score", value=f"**{a_goals} - {b_goals}**", inline=False)
    if narrative:
        # truncate if too long
        summary = narrative if len(narrative) <= 1000 else (narrative[:980] + "Ã¢â‚¬Â¦")
    ...
    if target.get("king") is None:
        if winner:
            target["king"] = winner
            target["streak"] = 1
            result_msg = f"Ã°Å¸â€˜â€˜ | **A New King is Crowned!** <@{winner}> wins the first battle! Ã°Å¸Ââ€  Streak: 1"
        else:
            result_msg = "Ã°Å¸Â¤Â It's a draw Ã¢â‚¬â€ no King yet."
    else:
        current_king = target.get("king")
        if winner is None:
            result_msg = "Ã°Å¸Â¤Â It's a draw! No change to the throne."
        elif winner == current_king:
            target["streak"] = target.get("streak", 0) + 1
            result_msg = f"Ã°Å¸â€˜â€˜ | **The King Defends the Throne!** <@{winner}> wins again! Ã°Å¸â€Â¥ Streak: {target['streak']}"
        else:
            prev = current_king
            target["king"] = winner
            target["streak"] = 1
            result_msg = f"Ã°Å¸â€˜â€˜ | **A New King is Crowned!** <@{winner}> dethrones <@{prev}>! Ã°Å¸Ââ€  Streak: 1"
    ...
    try:
        ...
        await ctx.send(f"Ã¢Å¡ Ã¯Â¸Â Warning: could not save KoTH state: {e}")
...
async def kingstatus(ctx):
    ...
    if chan in koth_draft and koth_draft[chan].get("active"):
        s = koth_draft[chan]
        if s.get("king"):
            await ctx.send(f"Ã°Å¸â€˜â€˜ Current Draft KoTH King: <@{s['king']}> | Ã°Å¸â€Â¥ Streak: {s.get('streak', 0)}")
        else:
    ...
    for sid, s in active.items():
        king = f"<@{s['king']}>" if s.get("king") else "Ã¢â‚¬â€ No King yet Ã¢â‚¬â€"
        lines.append(f"Ã¢â‚¬Â¢ ID `{sid}` Ã¢â‚¬â€ King: {king} Ã¢â‚¬â€ Streak: {s.get('streak', 0)} Ã¢â‚¬â€ Players: {len(s.get('players', []))}")
...
async def kothleaderboard(ctx):
    ...
    desc = "\n".join([f"{i+1}. <@{uid}> Ã¢â‚¬â€ {streak} defenses" for i, (uid, streak) in enumerate(items)])
    embed = discord.Embed(title="Ã°Å¸Ââ€  KoTH Leaderboard", description=desc, color=discord.Color.gold())
    await ctx.send(embed=embed)
...
async def koth_list(ctx, mode: str = None):
    ...
    if mode == "auction":
        ...
        if not active:
            await ctx.send("Ã¢Å¡ Ã¯Â¸Â No active Auction KoTH sessions in this channel.")
            return
        lines = []
        for sid, s in active.items():
            king = f"<@{s['king']}>" if s.get("king") else "No King yet"
            lines.append(f"Ã¢â‚¬Â¢ `{sid}` Ã¢â‚¬â€ King: {king} Ã¢â‚¬â€ Streak: {s.get('streak', 0)} Ã¢â‚¬â€ Players: {len(s.get('players', []))}")
        await ctx.send("**Active Auction KoTH sessions:**\n" + "\n".join(lines))
        return
    elif mode == "draftclash":
        s = koth_draft.get(chan)
        if not s or not s.get("active"):
            await ctx.send("Ã¢Å¡ Ã¯Â¸Â No active Draft Clash KoTH session in this channel.")
            return
        king = f"<@{s['king']}>" if s.get("king") else "No King yet"
        await ctx.send(f"**Draft Clash KoTH:** ID `{s.get('id')}` Ã¢â‚¬â€ King: {king} Ã¢â‚¬â€ Streak: {s.get('streak', 0)} Ã¢â‚¬â€ Players: {len(s.get('players', []))}")
        return
    else:
...
async def koth_lineup(ctx):
    ...
    embed = discord.Embed(
        title="Ã°Å¸Å½Â¯ Select Formation for KoTH",
...
async def viewteam(ctx):
    ...
    embed = discord.Embed(title=f"Ã¢Å¡Â¡ {ctx.author.display_name}'s Draft Clash Team", color=discord.Color.green())
